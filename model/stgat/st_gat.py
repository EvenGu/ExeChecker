...
from model.stgat.AdjMatrixGraph import AdjMatrixGraph
...
class STAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=8, num_node=25, num_frame=32,
                 ...
                 use_temporal_att=True, use_spatial_att=True, attentiondrop=0, use_pes=True, use_pet=True, skeleton=None): #, save_attn=None): #,writer=None)
        ...
        if use_spatial_att:
            ...
            if att_s:
                ...
                adj_graph = AdjMatrixGraph(skeleton, num_node)
                # A: normalize_adjacency_matrix
                self.graph_a = Variable(torch.from_numpy(adj_graph.A.astype(np.float32)), requires_grad=False)
                # A_sep: seperated_adjacency, hop-list: [0,2,4,6,8]
                self.graph = Variable(torch.from_numpy(adj_graph.A_sep.astype(np.float32)), requires_grad=False)
        ...
        if use_temporal_att:
            ...
            self.pet = PositionalEncoding(out_channels, num_node, num_frame)
        ...
        self.register_buffer('att_weights_spatial', None)
        # self.register_buffer('att_weights_spatial_after', None)
        self.register_buffer('att_weights_temporal', None)

    # return spatial attentions and temporal attentions, before global regularization
    def get_attention_weights_spatial(self):
        return self.att_weights_spatial # self.att_weights_spatial_after
    
    def get_attention_weights_temporal(self):
        return self.att_weights_temporal
    
    def forward(self, x):
        N, C, T, V = x.size()
        if self.use_spatial_att:
            attention = self.atts # shape: (1, num_subset(S), num_node(V)*self.window_size, num_node)
            # TODO:  use AdjMat as spatial positional embeding
            # if self.use_pes:
            #     y = self.pes(x)
            # else:
            #     y = x
            ...
            if self.att_s:
                ...
                k = self.in_nets(y).view(N, self.num_subset, self.inter_channels, T, V)  # nctv -> n s c' t v
                ...
                # save, suggested for visualization 
                # np.save('stgat_attention.npy', F.softmax(attention, dim=-1)[0].detach().cpu().numpy().copy()) 
                self.att_weights_spatial = F.softmax(attention, dim=-1).detach()

                zero_vec = -9e15*torch.ones_like(attention[:,0]) # mask
                self.graph = self.graph.cuda(x.get_device()) # (k-1, v, v)
                # selectively update attention by zeroing out connections that do not exist in the graph.
                attention = torch.cat([
                     torch.where(self.graph[i].repeat(self.window_size, 1) > 0, attention[:, i, :, :], zero_vec).unsqueeze(1) 
                     for i in range(self.num_subset)], 1) # n s window
                ...
                # save
                # self.att_weights_spatial_after = F.softmax(attention, dim=-1).detach()

            if self.glo_reg_s:
                # spatial attention w/ global regularization
        ...
        # <<<<<<<<<<< done with spatial attentions ----
        # ---- proceed with temporal >>>>>>>>>>>>>>>>>>

        if self.use_temporal_att:
            attention = self.attt # shape: (1, num_subset, num_frame, num_frame)
            if self.use_pet:
                z = self.pet(y)
            else:
                z = y
            if self.att_t:
                ...
                # save
                self.att_weights_temporal = F.softmax(attention, dim=-1).detach()

            if self.glo_reg_t: 
                # temporal attention w/ global regularization
...
class DSTANet_smp(nn.Module):
    def __init__(self, num_point=21, num_frame=32, num_subset=4, dropout=0., config=None, 
                 num_channel=3, glo_reg_s=True, att_s=True, glo_reg_t=True, att_t=True,
                 use_temporal_att=True, use_spatial_att=True, attentiondrop=0, dropout2d=0, use_pet=True, 
                 skeleton='execheck_skeleton'):
        super(DSTANet_smp, self).__init__()

        self.out_channels = config[-1][1] # 256
        in_channels = config[0][0] # 64
        ...
        # self.last_attention_block = self.graph_layers[-1]
        # self.last_attention_block.register_forward_hook(self.capture_attention)
        # self.att_weights_spatial = None
        # self.att_weights_temporal = None

    # def capture_attention(self, module, input, output):
    #     self.att_weights_spatial = module.get_attention_weights_spatial()
    #     self.att_weights_temporal = module.get_attention_weights_temporal()
    
    # only last graph layer
    def get_stgat_att_w_s(self):
        return self.graph_layers[-1].get_attention_weights_spatial() # pre_diff
    
    def get_stgat_att_w_t(self):
        return self.graph_layers[-1].get_attention_weights_temporal() 


    def forward(self, x):
        """

        :param x: N M C T V -- N is batch_size
        ...
        x = x.permute(0, 1, 3, 2).contiguous().view(N, -1, self.out_channels, M)  # whole channels of one spatial
        feat = self.drop_out2d(x[:,:,:,0].unsqueeze(-1))
        feat = feat.mean(1) # mean on CTV
        feat = self.drop_out(feat)  # whole spatial of one channel
        feat = feat.squeeze()
        if M>1:
            extra = x[:,:,:,1:].mean(1) 
            extra = extra.contiguous().view(N, -1)
        else:
            extra = None
        # class_scores = self.fc(x)
        return feat, extra
...
if __name__ == '__main__':
    ...
    net = DSTANet_smp(config=config).cuda()
    ske = torch.rand([2, 3, 32, 21, 2]).cuda() # NCTVM
    print(net(ske).shape)
